...
{
     public class 先驱怒火 : ModProjectile
    {

        public override void SetDefaults()
        {
            Projectile.width = 69; //设置弹幕的宽度
...
           if(Projectile.timeLeft <= 500 &&  Projectile.timeLeft >= 499)
          {
            Projectile.position = playerPosition + new Vector2(Main.rand.Next(-900, 900), Main.rand.Next(-900, 900));
            var dust = Dust.NewDustDirect(position, 0, 0, 87, 0, 0, 50, Color.White, 3f);
           dust.noGravity = true;   
          }
          if(Projectile.timeLeft <= 499 &&  Projectile.timeLeft >= 399)
          {
            // 旋转点速率可以自行调整
            var dust = Dust.NewDustDirect(position, 0, 0, 87, 0, 0, 200, Color.White, 3f);
           dust.noGravity = true;   
          ...
          {
            var dust = Dust.NewDustDirect(position, 0, 0, 87, 0, 0, 100, Color.White, 2f);
           dust.noGravity = true;   
