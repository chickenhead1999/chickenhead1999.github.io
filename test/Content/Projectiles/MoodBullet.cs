...
{
    public class MoodBullet : ModProjectile
    {
       ...
       {
        Vector2 targetPosition = Main.MouseWorld; // 获取鼠标在游戏世界的位置
        Vector2 direction = targetPosition - Projectile.position; // 计算弹幕移动方向
        direction.Normalize();
        Projectile.velocity = direction * 20f;
        Vector2 position = Projectile.position;

        var dust = Dust.NewDustDirect(position, 1, 1, 101, 0, 0, 100, Color.White, 2f);
        dust.noGravity = true;
       }
}
}
